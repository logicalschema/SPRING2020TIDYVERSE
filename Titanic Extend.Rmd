---
title: "tidyverse"
author: Gehad Gad via "Subhalaxmi Rout & Neil Shah"
date: "4/19/2020"
...
titanic_data <- titanic_data %>% select(-Ticket,-Fare,-Cabin,-Parch)
...
```{r}
#Set Embarked and Survived as.factor.
...
```{r}
#Display a graph for the survival rate by emabarked
ggplot(titanic_data,aes(x= Embarked,fill= Survived)) +
theme_bw() +
geom_bar()+
labs(y ="Passenger Count",
title = "Titanic Survival Rates by Embarked")
```

We can see that Port of Embarkation (C = Cherbourg; Q = Queenstown; S = Southampton) feature has a significat impact to the survival rate. There are more people whom Embarked from (Southampton) did not survived. Followed by Embarked (Cherbourg) and (Queenstown) is the least. This gives a future reference.




```{r}
#Graph the survival rate for people with Siblings/Spouses Aboard.

ggplot(titanic_data,aes(x= SibSp,fill= Survived)) +
theme_bw() +
geom_bar()+
labs(y ="Passenger Count",
title = "Titanic Survival Rates by sibSp")


```

This graph above shows the survival rate based on people with Siblings/Spouses.Individuals without siblings or spouse survive more than individuals who accompany their family. This is an important factor since it is difficult for any person to survive if they have family.

```{r}
#Graph the survival rate by Age.
...
This graph shows the survival rate by Age. 

```{r}
ggplot(titanic_data,aes(x= Age, fill = Survived)) +
theme_bw() +
facet_wrap(Sex ~ Class_Type)+
geom_density(alpha = 0.5)+
labs(y ="Age",
x = "Survived",
title = "Titanic Survival Rates by Age, class type, and Sex")
```

This a good graph to show the survival rate by Age, Class type and Sex.
